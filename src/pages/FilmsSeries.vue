<template>
  <div class="min-h-screen bg-cover bg-gray-200 text-center " :style="{ backgroundImage: 'url(' + require('@/images/filmsWallpaper.webp') + ')' }">
    ...
    <div class="mt-8 mx-auto w-[80%] lg:w-[30rem]">
      <input v-model="movieTitle" type="text" class="border border-gray-300 rounded px-[1rem] py-[0.5rem] mb-[1rem] w-full" placeholder="Enter movie title">
      <CustomButton  :title="searchTitle" @click="searchMovie" class="bg-bluePurple hover:bg-gray-900 text-white font-medium rounded font-montserrat ease-out duration-300 lg:w-[45%] mb-[1rem] lg:mb-0 mx-auto"/>
    </div>

    <!-- Display movie information -->
    <div></div>
    <div v-if="movieInfo" class="mt-[5rem] mx-auto lg:w-[60%] w-[80%] bg-white bg-opacity-85 font-montserrat p-[3rem] rounded-[1rem] rounded-br-none">
      <h4 class="font-bold text-[1.25rem]">{{ movieInfo.Title }}</h4>
      <p>{{ movieInfo.Plot }}</p>
      <div class="lg:flex mx-auto lg:w-[90%] xl:w-[80%] lg:justify-center justify-between hidden">
        <div class="flex flex-col font-montserrat font-bold text-start mr-[0.2rem]">
          <p>Director: </p>
          <p>Actors: </p>
          <p>Year: </p>
          <p>Rating: </p>
