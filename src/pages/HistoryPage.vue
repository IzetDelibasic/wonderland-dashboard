<template>
  <div :style="{ backgroundImage: 'url(' + require('@/images/dashboardBackground.webp') + ')', backgroundSize: 'cover' }" class="min-h-screen">
    ...
    <div class="w-[60%] flex flex-col justify-center items-center mx-auto mt-[3rem] bg-white bg-opacity-80 rounded-[30px] rounded-bl-none rounded-tr-none hover:bg-opacity-45 hover:rounded-[30px] ease-in-out duration-300 p-[5rem]">
      <div class="xl:h-[25rem]">
        <div v-if="currentCharacter" class="">
          <div v-for="item in currentCharacter.items" :key="item.name" class="flex flex-col justify-center items-center text-center">
            ...
            <p class="w-[50%] py-[0.5rem] font-montserrat lg:text-[18px] font-medium">{{ item.history }}</p>
          </div>
        </div>
        <div v-else>
      ...
      <div class="flex flex-row justify-between w-[40%] pt-[10rem]">
        <CustomButton @click="previousCharacter" :title="buttonOneText" class="bg-gray-900 hover:bg-bluePurple text-white font-medium rounded font-workSans ease-out duration-300 w-[40%]"/>
        <CustomButton @click="nextCharacter" :title="buttonTwoText" class="bg-gray-900 hover:bg-bluePurple text-white font-medium rounded font-workSans ease-out duration-300 w-[40%]"/>
      </div>
    </div>
  </div>
</template>
